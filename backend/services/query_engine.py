...
def process_course_query(query: str, user_data: str) -> dict:
    """
    ...
    # Extract structured course data from documents
    course_data = []
    for doc in unique_results[:5]:
        # Extract course details from metadata
        course_info = {
            "course_code": doc.metadata.get('class_code', ''),
            "course_name": doc.metadata.get('class_name', ''),
            "credits": doc.metadata.get('credits', ''),
            "description": doc.metadata.get('description', ''),
            "prerequisites": doc.metadata.get('prerequisites', ''),
            "instructor": doc.metadata.get('instructor', ''),
            "schedule": {
                "days": doc.metadata.get('days', ''),
                "time": doc.metadata.get('time', '')
            },
            "location": doc.metadata.get('classroom', ''),
            "availability": {
                "available_seats": doc.metadata.get('available_seats', ''),
                "total_seats": doc.metadata.get('total_seats', '')
            },
            "crn": doc.metadata.get('crn', '')
        }
        course_data.append(course_info)
    
    # Generate the conversational part of the response
    ...
    conversational_message = final_response.content.strip()
